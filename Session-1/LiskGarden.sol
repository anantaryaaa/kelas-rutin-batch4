...
contract LiskGarden {
    ...
    uint256 public constant REWARD_PANEN = 0.003 ether;
    ...
    uint256 public WATER_DEPLETION_TIME = 30 seconds;
    ...
    function plantSeed() external payable returns (uint256) {
        ...
        require(msg.value >= PLANT_PRICE, "Insufficicent balance");
        plantCounter++;
        ...
        emit PlantSeeded(plants[plantCounter].owner, plants[plantCounter].id);
        return plantCounter;
    }
    ...
    function calculateWaterLevel(uint256 _plantId) public view returns (uint256) {
        ...
        return plants[_plantId].waterLevel - waterLost;
    }
    ...
    function waterPlant(uint256 _plantId) external {
        // TODO: Implement
        require(plants[_plantId].exist, "Plant does not exist");
        require(msg.sender == plants[_plantId].owner, "Not the owner of the plant");
        require(!plants[_plantId].isDead, "Plant is already dead");
        plants[_plantId].waterLevel = 100;
        plants[_plantId].lastWatered = block.timestamp;
    ...
    function updatePlantStage(uint256 _plantId) public {
        ...
        if (plants[_plantId].isDead) {
            return;
        }
        uint256 timeSincePlanted = block.timestamp - plants[_plantId].plantedDate;
        // GrowthStage oldStage = plants[_plantId].stage;
        if (timeSincePlanted >= 1 minutes) {
            plants[_plantId].stage = GrowthStage.SPROUT;
            // emit stage(plantId);
        } else if (timeSincePlanted >= 2 minutes){
            plants[_plantId].stage = GrowthStage.GROWING;
            // emit stage(plantId);
        } else if (timeSincePlanted >= 3 minutes) {
            plants[_plantId].stage = GrowthStage.BLOOMING;
            // emit stage(plantId);
        } else{
            plants[_plantId].stage = GrowthStage.SEED;
            // emit stage(plantId); 
        }
    }
    ...
    function harvestPlant(uint256 _plantId) external {
        // TODO: Implement
        require(plants[_plantId].exist, "Plant does not exist");
        require(msg.sender == plants[_plantId].owner, "Bukan owner");
        require(!plants[_plantId].isDead, "Plant is dead");
        updatePlantStage(_plantId);
        require(plants[_plantId].stage == GrowthStage.BLOOMING, "Plant is not blooming");
        plants[_plantId].exist = false;
        emit PlantHarvested(_plantId, msg.sender, REWARD_PANEN);
        (bool success, ) = msg.sender.call{value: REWARD_PANEN}("");
